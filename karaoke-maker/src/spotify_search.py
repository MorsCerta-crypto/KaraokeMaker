...
class SpotifySearch:
    ...
    def from_search_term(self, query: str) -> list[Song]:
        ...
        try:
            song = self.from_spotify_url(song_url, self.output_format)
            return [song] if song.youtube_link is not None else []
        except (LookupError, OSError, ValueError):
            return []

    def from_spotify_url(self, spotify_url: str, output_format: str = "mp3") -> Song:
        ...
        converted_file_path = Path(
            "data/downloads", f"{converted_file_name}.{output_format}"
        )
...
if __name__ == "__main__":
    s = SpotifySearch()
    s.from_search_term("ballad of hollywood jack and the rage cage")
